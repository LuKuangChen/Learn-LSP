...
import { disconnect, off, rawListeners } from 'process';
...
type Term = Variable<Term> | Abstraction<Term> | Application<Term> | Binding<Term>
type TermNode = Variable<number> | Abstraction<number> | Application<number>
...
interface Abstraction<Term> {
	kind: "fun";
	name: Variable<Term>
	body: Term
}
interface Application<Term> {
	kind: "app";
	operator: Term
	operand: Term
}
interface Binding<Term> {
	kind: "let";
	name: Variable<Term>;
	init: Term;
	body: Term;
...
const Port = {
	...
	rest: ({ all, index, pos: { line, character } }: Port) => {
		if (index < all.length) {
			index = index + 1;
			if (all[index - 1] === "\n") {
				line = line + 1;
	...
	checkFirst(p: Port, f: (c: string) => boolean) {
...
function parse(code: string): Term {
	let port: Port = Port.make(code);
	function parseVariable(): Variable<Term> {
	...
	function eat(n: number) {
		for (; n > 0; n--) {
			port = Port.rest(port);
		}
		return;
	}
	function assertNext(s: string) {
...
const documentContents: Map<string, Term> = new Map();
...
async function validateTextDocument(textDocument: TextDocument): Promise<void> {
	...
	if (term) {
		const table: Array<{ from: Variable<Term>, to: Variable<Term> | null }> = [];
		const walk = (t: Term, env: Map<string, Variable<Term>>) => {
			if (t.kind == "var") {
...
function prettyPrint(term: Term): string {
	function indent(offset: number): string {
		if (offset === 0) {
			return "";
		} else {
	...
	function ppTerm(offset: number, term: Term): string {
		if (term.kind === "var") {
			return term.name;
...
connection.onDocumentFormatting(
	(params, token, workDoneProgress, resultProgress) => {
		...
		const formattedText = prettyPrint(term as Term);
...
connection.onDefinition((params) => {
	...
	const term: Term = documentContents.get(uri) as Term;
	const table: Array<{ from: Variable<Term>, to: Variable<Term> | null }> = [];
	function walk(t: Term, env: Map<string, Variable<Term>>) {
		if (t.kind == "var") {
			table.push({ from: t, to: env.get(t.name) || null });
		} else if (t.kind == "fun") {
			table.push({ from: t.name, to: t.name });
